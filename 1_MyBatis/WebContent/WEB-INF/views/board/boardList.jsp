	...
	<c:if test="${ !empty loginUser }">
		<div class="outer">
			...
			<table id="boardTable">
				...
				<%-- <c:forEach begin="0" end="${list.size()}" items="${ list }" var="b"> --%>
	 			<c:forEach items="${ list }" var="b"> <!-- begin과 end는 설정 안해도 됨 -->
					<tr>
						<td>${ b.bId }</td>
						<td>${ b.bTitle }</td>
						<td>${ b.nickName }</td>
			...
			<div class="pagingArea" align="center">
				<!-- 맨 처음 -->
				<c:if test="${ pi.currentPage <= 1 }">[처음]</c:if>
				...
				<c:if test="${ pi.currentPage > 1 }">
<%-- 					<c:url value="selectList.bo" var="blistBack">  --%><!-- url을 직접 써주는게 아니라 c:url태그를 이용 -->
				...
				<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
					<c:if test="${ p eq pi.currentPage }"><font color="red" size="4">[${ p }]</font></c:if> 
				...
				<!-- 맨 끝 -->
				<c:if test="${ pi.currentPage >= pi.maxPage }">[끝]</c:if>
				<c:if test="${ pi.currentPage < pi.maxPage }">
